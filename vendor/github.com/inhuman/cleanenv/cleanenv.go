...
const (
	...
	// TagEnvJSON flag to unmarshal env value as JSON into the field (struct, slice, map, ptr)
	TagEnvJSON = "env-json"
)
	...
	for i := 0; i < len(cfgStack); i++ {
		...
		for idx := 0; idx < s.NumField(); idx++ {
			...
			if fld := s.Field(idx); fld.Kind() == reflect.Struct {
				...
				// add structure to parsing stack (unless env-json:"true" — handled as JSON below)
				if _, found := validStructs[fld.Type()]; !found {
					if fType.Tag.Get(TagEnvJSON) != "true" {
			...
			metas = append(metas, StructMeta{
				...
				updatable:   upd,
				required:    required,
				isJSON:      fType.Tag.Get(TagEnvJSON) == "true",
			})
		}
...
func readEnvVars(cfg interface{}, update bool) error {
	...
	for _, meta := range metaInfo {
		...
		if meta.isJSON {
			...
			target := meta.fieldValue.Addr().Interface()
			if meta.fieldValue.Kind() == reflect.Ptr {
				if meta.fieldValue.IsNil() {
					meta.fieldValue.Set(reflect.New(meta.fieldValue.Type().Elem()))
				}
				target = meta.fieldValue.Interface()
			}
